...
def sort_by_number_of_routes(routes: list): #sort by number of routes
    def order_routes(route: ClimbingArea):
            return route.routes() #get the len of route (all routes)
    return sorted(routes, key=order_routes, reverse=False) #sort them

def sort_by_most_difficult(routes: list):
    def order_difficult(route: ClimbingArea):
            return route.hardest_route().grade #get from routes hardest route grade
    return sorted(routes, key=order_difficult, reverse=True)
